...
const examples = [
  'Can you please tell me more about the Amazon and itsenvironment?',
  'What would you say are the most important skills needed to be successful in this role?',
  'Can you help me prepare for the Longshot interview',
  'What are the basic needs to survive in a jungle alone?'
];
...
const Longshot = () => {
  ...
  const handleSend = async () => {
    ...
    // Inside the try block, after receiving the response
      ...
      setChat(currentChat => [...currentChat, { role: 'assistant', content: data }]);
      } else if (data && data.copies && Array.isArray(data.copies)) {
  const aiResponse = data.copies.map(item => item.content).join('\n');
  setChat(currentChat => [...currentChat, { role: 'assistant', content: aiResponse }]);
}
  ...
  const handleDeleteChat = (index) => {
    const updatedChatHistory = chatHistory.filter((_, idx) => idx !== index);
    setChatHistory(updatedChatHistory);
  };
  const startNewChat = () => {
    setChat([]);
    const newTitle = `Chat - ${chatHistory.length + 1}`;
    setTitle(newTitle);
    setChatHistory(prevHistory => [...prevHistory, { title: newTitle, chat: [] }]);
  };
  ...
  return (
    <div className="h-screen w-screen flex bg-[#050509] body">
      <div className="w-[20%] h-screen bg-[#0c0c15] text-white p-4 overflow-y-hidden hide-scroll gradient-04 glassmorphism-2" onClick={()=> {
        ...
        <div>
          <button className="w-full h-[50px]  over bg-discount-gradient rounded" onClick={startNewChat}>
            + New Interview
          </button>
        </div>
        <div className=' h-[70%] overflow-scroll shadow-lg hide-scroll-bar mb-4 '>
          {
            chatHistory.map((item, index) => (
              <div className=' py-3 text-center rounded mt-4 text-lg font-light flex items-center px-8 hover:bg-slate-600 cursor-pointer'>
                ...
                <button onClick={() => handleDeleteChat(index)} className="ml-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded text-base">
                Delete
              </button>
              </div>
            ))
      ...
      <div className="w-[80%] gradient-05">
        {chat.length > 0 ? (
          ...
          <div className="h-[80%]  flex flex-col justify-center items-center text-white gradient-04 ">
            <div className="text-4xl font-bold mb-8">Longshot-AI</div>
